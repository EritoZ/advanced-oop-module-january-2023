...
class Television(PowerOutlet, HDMICable, RCACable):
...
class DVDPlayer(PowerOutlet, HDMICable):
...
class GameConsole(PowerOutlet, EthernetCable, HDMICable):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(PowerOutlet, EthernetCable):
    def connect_to_tv(self, television):
